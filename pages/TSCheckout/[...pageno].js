...
export async function getServerSideProps(context) {
    ...
    const response = await fetch(`${process.env.API_URL}Openendpoint/GetTsbyid`, requestOptions);
...
function Overview({ CourseFullData, PAYTM_MID, PAYTM_HOST }) {
        ...
        if (Contextdata.IsLogin == true) {
            if (CourseFullData.CourseData.length > 0) {
                ...
                if (DataMian.isFree === false) {
                    setOrderID('DRITPT24' + RandA + RandB);
                ...
                if (DataMian.isFree === true) {
                    setOrderID('DRITFT24' + RandA + RandB);
    ...
    const CreateOrderBTN = async () => {
        if (Contextdata.IsLogin == true) {
            ...
            const data = fetch("/api/V2/TSOrders/FreeCheckout", {
                method: "POST",
                headers: {
                ...
                .then((parsedIfAvail) => {
                    ...
                    setTimeout(function () {
                        ...
                        if (parsedIfAvail.ReqData.done) {
                            ...
                            notify('😎 Free Test Series Successfully Added to your Learning Dashboard')
                            setTimeout(function () {
                                router.push('/MyTS')
                            }, 2000);
                        } else {
    ...
    const CreateOrderPaid = async () => {
        if (Contextdata.IsLogin == true) {
            ...
            const encryptedData = CryptoJS.AES.encrypt(JSON.stringify(sendUser), CryptoJSKEY).toString();
            const SendFinalData = { encryptedData: encryptedData }
            const data = fetch("/api/V2/TSOrders/CreateTSSalesOrder", {
                method: "POST",
                headers: {
                    'Content-type': 'application/json'
                },
    ...
    const UpdateOrder = async (DataRec) => {
        ...
        const SendFinalData = { encryptedData: encryptedData }
        const data = fetch("/api/Paytm/TS/posttransaction", {
            method: "POST",
            headers: {
                'Content-type': 'application/json'
            },
            ...
            .then((parsedUpdated) => {
                ...
                if (parsedUpdated.ReqData.done) {
                    notify('😎 Test Series Successfully Added to your Learning Dashboard')
                    setTimeout(function () {
                        router.push('/MyTS')
                    }, 3000);
                } else {
    ...
    const initiatePayment = async (e) => {
        ...
        const a = await fetch("/api/Paytm/TS/pretransaction", {
            method: "POST",
            headers: {
                'Content-type': 'application/json'
            },
                ...
                <div>
                    ...
                    {DataMian && DataMian.isFree === false &&
                        ...
                        <div>
                            <div style={{ minHeight: '20px' }}></div>
                            <div className={Mstyles.HeroBoxTwoTitle}>
                                <h1><span className={Mstyles.primaryColor}>Test Series</span> Checkout</h1>
                                <span>Please complete your order</span>
                            </div>
                            <div style={{ minHeight: '30px' }}></div>
                            <div className={Mstyles.CheckoutContainer}>
