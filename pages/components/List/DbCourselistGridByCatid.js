...
function RecentOrders({ catid }) {
    ...
    const GetData = async () => {
        ...
        const data = await fetch("/api/V3/List/DbCourselistbycatid", {
            method: "POST",
            headers: {
            ...
            .then((parsed) => {
                ...
                if (parsed.ReqD.CoursesList.length > 0) {
                    setRetdata(parsed.ReqD.CoursesList)
            ...
            <div>
                    ...
                    <div>
                            ...
                            <div className={MYS.DbContainerpadding}>
                                <div>
                                    <div className={Mstyles.HeroBoxTwoTitle}>
                                        <h1>Best Courses for <span className={Mstyles.HeroBoxTwoTitleBottom}> {Contextdata.MainCat.name}</span></h1>
                        ...
                        {Retdata.length > 0 &&
                            <div className={MYS.DbContainerpaddingOnlyDesktop}>
                                <div className={Mstyles.DbCourseGrid}>
                                    {Retdata.map((item) => {
                                        return <div className={Mstyles.DbCourseItems} key={item._id} onClick={() => router.push(`/course/${item.pid}`)}>
                                            <div
                                                style={{
                                                    position: "relative",
                                                    width: "100%",
                ...
                {Retdata.length > 3 &&
                    <div className={Mstyles.Loadmorebtnbox}>
                        <LoadingButton
                            fullWidth
                            onClick={() => router.push(`/Courses/${catid}`)}
                            endIcon={<FiChevronRight />}
                            loading={false}
                            loadingPosition="end"
                            variant="outlined"
