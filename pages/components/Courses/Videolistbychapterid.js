...
function RecentOrders({ chapterid, pid }) {
    ...
    const GetData = async () => {
        ...
        const data = await fetch("/api/V3/Students/Videolistbychapterid", {
            method: "POST",
            headers: {
            ...
            <div>
                {!isLoading &&
                    <div>
                        ...
                        {Retdata.length == 0 &&
                            <Nodatafound
                                Title={'No Vidieo Lecture Found'}
                                Desc={'We are adding Video Lectures to this Chapter please stay tuned and try after some time'}
                        ...
                        {Retdata.length > 0 &&
                            <div>
                                {Retdata.map((item, index) => {
                                    return <div key={index} onClick={() => router.push(`/WatchVideo/${item._id}`)}>
                                        <div className={MYS.VideolistboxItem}>
                                            <div className={MYS.VideolistboxItemA}>
                                            ...
                                            <div className={MYS.VideolistboxItemB}>
                                                <div className={MYS.OnlyDesktop}>
                                                    <IconButton aria-label="cart" onClick={() => router.push(`/WatchVideo/${item._id}`)}>
